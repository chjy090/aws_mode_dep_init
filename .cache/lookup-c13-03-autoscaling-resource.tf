...
resource "aws_autoscaling_group" "asg_apply_name_lookup" {
  name_prefix = "asg_${var.apply_name_lookup}"
  desired_capacity = 1
  max_size = 10
  min_size = 1
  ...
  launch_template {
    id = aws_launch_template.apply_name_lookup_launch_template.id 
    version = aws_launch_template.apply_name_lookup_launch_template.latest_version
  }
  # Instance Refresh
  instance_refresh {
    strategy = "Rolling"
